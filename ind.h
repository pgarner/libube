 ...
 * evaluated in a boolean context, it returns false if the value is negative
 * and true otherwise.  This is in contrast to a normal C integer type that
 * returns false if the value is 0 and true otherwise.  Hence, it functions as
 * a postive integer.  The default (undefined) value is zero, hence true.
 *
 ...
 *
 * All other defined operators do the same as would be expected for an integer
 * type.
 */
class ind {
public:
    ...
    explicit operator bool() { return mInt < 0 ? false : true; };
    operator intptr_t() { return mInt; };
